...
- Learn the core features of nf-core.
...
For this section of the workshop we will be using the `nf-core/testpipeline` as an example. We will also create a separate directory for this section.
...
### **3.2.5. Viewing workflow parameters**
...
In the example above, the parameters for `rnaseq` pipeline version `3.18.0` is being viewed. 
...
### **3.2.9. Default parameters and configuration fiiles**
...
```default
nextflow run nf-core/<pipeline> test,singularity
```
...
**Exercise**: Use the command line to set the [MultiQC report tile](https://github.com/nf-core/testpipeline/blob/master/nextflow.config#L23) as your **favourite animal**. 

::: {.callout-note collapse="true"}
### Hint

To change the MultiQC report title, the `--multiqc_title` parameter can be used.
...
```default
nextflow run -r 2.0 nf-core/testpipeline -resume -profile test,<SOFTWARE> --outdir 'output_testpipeline' --multiqc_title 'koala'
```
:::
...
### **3.2.7. Custom configuration files**

Configuration `.config` files contain various workflow properties that can also be specified in the command-line, via the `-c` option:
...
Similarly, the `<CONTAINER>` scope controls how <CONTAINER> containers are executed by Nextflow. In this example, the <CONTAINER> container software management has been enabled. 
...
**Exercise**: Set the [MultiQC report tile](https://github.com/nf-core/testpipeline/blob/master/nextflow.config#L23) as your **favourite colour**, using a custom `custom.config` file. 
...
```default
nextflow run -r 2.0 nf-core/testpipeline -resume -profile test,<SOFTWARE> --outdir 'output_testpipeline' -c custom.config
```
:::
...
```default
nextflow run -r 2.0 nf-core/testpipeline -resume -profile test,<SOFTWARE> --outdir 'output_testpipeline' -params-file my_params.yml 
```
:::
