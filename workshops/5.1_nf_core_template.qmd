...
Comparing this to the simple rnaseq.nf we created from [previous session](./3.1_creating_a_workflow.qmd), the structure is quite different. 
...
[08/3c1cfa] process > MULTIQC (3)        [100%] 3 of 3 ✔
``` 

Adding the `tag` directive, we can change how this is being displayed. 
...
```default
nextflow run rnaseq.nf --outdir myresults -resume
```

The following will be printed, where the `FASTQC` process names now contain the `tag` that has been defined (pay attention to how the tag is updated as the the number of processes completed).
...
[66/2ef36a] process > MULTIQC (3)        [100%] 3 of 3 ✔
```

In the `FASTQC` process, the tag has been defined as `$meta.id`, which we will go through further in [later section](./5.3_metadata_propagation.qmd) how nf-core utilises the `meta` variable for metadata propagation. 
...
[/home/larigan/lesson3.2/work/e3/43da722e6feb27ee4bc4c04b73e01a/fastqc_gut_logs/gut_1_fastqc.zip, /home/larigan/lesson3.2/work/e3/43da722e6feb27ee4bc4c04b73e01a/fastqc_gut_logs/gut_2_fastqc.zip]
```

::: {.callout-note collapse="false"}
### Quiz

What will happen if we did not update `multiqc_ch = MULTIQC(quant_ch, FASTQC.out)` to `multiqc_ch = MULTIQC(quant_ch, FASTQC.out[0])`?
...
Process `MULTIQC` declares 2 input channels but 3 were specified
```
:::
...
If we observe within the `fastq_fastqc_umitools_fastp` [subworkflow file](https://github.com/nf-core/rnaseq/blob/b96a75361a4f1d49aa969a2b1c68e3e607de06e8/subworkflows/nf-core/fastq_fastqc_umitools_fastp/main.nf#L5), the `FASTQC` process is imported using the following command:
...
Also download the following test data to your folder, if you haven't already done it from the previous workshop:
...
The [nf-core FASTQC process](https://github.com/nf-core/modules/blob/master/modules/nf-core/fastqc/main.nf) has the `process_medium` label, which by default required 6 CPUs. Previously when we were running nf-core pipelines, we would use the `--max-cpus` and `--max-memory` to cap the resources, but in this case, because this is our own workflow, we will update it in our base config. 
...
### **Next Chapter: [nf-core modules](./5.2_nf_core_modules.qmd)** 
