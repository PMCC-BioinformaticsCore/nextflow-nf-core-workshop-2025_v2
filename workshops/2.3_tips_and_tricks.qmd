...
Nextflow, by default, launches mulitple parallel tasks that can be ran concurrently. Recall previously that we used the `process` and `executor` scope to run these tasks using an HPC job scheduler [SLURM](https://www.nextflow.io/docs/latest/executor.html#slurm), submitting a maximum number of 4 concurrent jobs. 

::: {.callout-important}
**TO DO**
Change EXECUTOR + PARTITION
:::
...
### **2.3.4. Change the default Nextflow cache strategy**
